...
export const toggleDieStatus = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const updated = await (prisma as any).dieMaster.update({ where: { id }, data: { is_active: !existing?.is_active } });
    const { logChange } = await import('../utils/audit');
    await logChange(existing?.tenant_id || '', 'die', id, existing?.die_number || '', existing?.is_active ? 'deactivate' : 'activate', existing, updated, req.user?.user_id || '', req.user?.email || '', reason);
