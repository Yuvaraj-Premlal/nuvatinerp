...
export const getFactoryStatus = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const machineStatus = await Promise.all(
      machines.map(async (machine: any) => {
        const activeJob = await prisma.jobCard.findFirst({
        ...
        const totalShots = todayJobs.reduce((sum: number, j: any) => sum + j.shot_logs.length, 0);
        const goodShots = todayJobs.reduce((sum: number, j: any) => sum + j.actual_quantity_good, 0);
        const totalDowntime = todayJobs.reduce((sum: number, j: any) =>
        ...
        return {
          ...
          current_job: activeJob ? {
            ...
            current_operation: activeJob.job_operations.find((op: any) => op.status === 'in_progress')?.operation_name,
    ...
    const dieStatus = dies.map((die: any) => {
    ...
    const stockItems = await prisma.itemMaster.findMany({
      where: { tenant_id, is_active: true },
    ...
    const materialStatus = stockItems.map((item: any) => {
      const ledger = stockLedger.find((l: any) => l.item_id === item.id);
    ...
    const activeJobCards = await prisma.jobCard.findMany({
      where: { tenant_id, status: { in: ['planned', 'released', 'in_progress'] } },
      include: { job_operations: { orderBy: { operation_sequence: 'asc' } } },
      orderBy: { planned_date: 'asc' }
    });
    const jobFlow = activeJobCards.map((job: any) => {
      const currentOp = job.job_operations.find((op: any) => op.status === 'in_progress');
      ...
      return {
        ...
        operations: job.job_operations.map((op: any) => ({
    ...
    const dispatchToday = await prisma.salesOrder.findMany({
    ...
    const latestBuffer = constraint ? await prisma.bufferLog.findFirst({
    ...
    res.json({
      ...
      data: {
        ...
        factory_summary: {
          ...
          machines_in_production: machineStatus.filter((m: any) => m.status === 'in_production').length,
          machines_idle: machineStatus.filter((m: any) => m.status === 'idle').length,
        ...
        job_flow: jobFlow,
        dispatch_due_today: dispatchToday.map((so: any) => ({
