...
exports.getJournalEntries = exports.getCashPosition = exports.getGSTSummary = exports.getSupplierAging = exports.getCustomerAging = exports.getProfitAndLoss = exports.reverseExpense = exports.createExpense = exports.getExpenses = exports.createDebitNote = exports.createPaymentVoucher = exports.createSupplierBillFromGRN = exports.getSupplierBills = exports.createCreditNote = exports.reversePaymentReceipt = exports.createPaymentReceipt = exports.getPaymentReceipts = exports.updateInvoiceStatus = exports.createInvoiceFromDispatch = exports.createInvoice = exports.getInvoiceById = exports.getInvoices = exports.createBankAccount = exports.getBankAccounts = exports.setCompanyConfig = exports.getCompanyConfig = void 0;
const prisma_1 = __importDefault(require("../config/prisma"));
// ─── COMPANY CONFIG ──────────────────────────────────────────────────────────
const getCompanyConfig = async (req, res) => {
    try {
        const tenant_id = req.user?.tenant_id;
        const config = await prisma_1.default.companyConfig.findUnique({ where: { tenant_id } });
...
const setCompanyConfig = async (req, res) => {
    try {
        const tenant_id = req.user?.tenant_id;
        const config = await prisma_1.default.companyConfig.upsert({
            where: { tenant_id },
            update: req.body,
...
exports.setCompanyConfig = setCompanyConfig;
// ─── BANK ACCOUNTS ───────────────────────────────────────────────────────────
const getBankAccounts = async (req, res) => {
    try {
        const tenant_id = req.user?.tenant_id;
        const accounts = await prisma_1.default.bankAccount.findMany({ where: { tenant_id, is_active: true } });
...
const createBankAccount = async (req, res) => {
    try {
        const tenant_id = req.user?.tenant_id;
        const account = await prisma_1.default.bankAccount.create({ data: { tenant_id, ...req.body } });
...
const getInvoiceById = async (req, res) => {
    try {
        ...
        const invoice = await prisma_1.default.invoiceHeader.findFirst({
...
const createInvoice = async (req, res) => {
    try {
        ...
        const count = await prisma_1.default.invoiceHeader.count({ where: { tenant_id } });
        ...
        const invoice = await prisma_1.default.invoiceHeader.create({
            data: {
                tenant_id,
                invoice_number,
                ...header,
...
const createInvoiceFromDispatch = async (req, res) => {
    try {
        ...
        const salesOrder = dispatch.so_id
            ? await prisma_1.default.salesOrder.findFirst({ where: { id: dispatch.so_id } })
            : null;
        const customer = salesOrder?.customer_id
            ? await prisma_1.default.customerMaster.findUnique({ where: { id: salesOrder.customer_id } })
            : null;
        const config = await prisma_1.default.companyConfig.findUnique({ where: { tenant_id } });
        const factoryStateCode = config?.state_code || '33';
        const customerStateCode = customer?.state_code || '33';
        const isInterState = factoryStateCode !== customerStateCode;
        const lines = dispatch.dispatch_lines.map((dl) => ({
            item_id: dl.item_id,
        ...
        req.body = {
            ...
            customer_name: salesOrder?.customer_name || customer?.customer_name || '',
        ...
        return (0, exports.createInvoice)(req, res);
    }
    catch (error) {
        res.status(500).json({ success: false, error: error.message });
    }
};
exports.createInvoiceFromDispatch = createInvoiceFromDispatch;
const updateInvoiceStatus = async (req, res) => {
    try {
        const tenant_id = req.user?.tenant_id;
        const id = req.params.id;
        const { status } = req.body;
        await prisma_1.default.invoiceHeader.updateMany({ where: { id: id, tenant_id: tenant_id }, data: { status: status } });
...
const createPaymentReceipt = async (req, res) => {
    try {
        ...
        const invoice = await prisma_1.default.invoiceHeader.findFirst({ where: { id: invoice_id, tenant_id } });
        if (!invoice)
            return res.status(404).json({ success: false, error: 'Invoice not found' });
        const count = await prisma_1.default.paymentReceipt.count({ where: { tenant_id } });
        const receipt_number = `RCP-${new Date().getFullYear()}-${String(count + 1).padStart(4, '0')}`;
        const receipt = await prisma_1.default.paymentReceipt.create({
            data: {
                tenant_id,
                receipt_number,
        ...
        await prisma_1.default.invoiceHeader.updateMany({
            where: { id: invoice_id },
...
const reversePaymentReceipt = async (req, res) => {
    try {
        const tenant_id = req.user?.tenant_id;
        const id = req.params.id;
        const { reason } = req.body;
        const receipt = await prisma_1.default.paymentReceipt.findFirst({ where: { id: id, tenant_id: tenant_id } });
        if (!receipt)
            return res.status(404).json({ success: false, error: 'Receipt not found' });
        if (receipt && receipt.status === 'reversed')
            return res.status(400).json({ success: false, error: 'Already reversed' });
        await prisma_1.default.paymentReceipt.updateMany({
            where: { id: id },
...
const createCreditNote = async (req, res) => {
    try {
        ...
        const count = await prisma_1.default.creditNote.count({ where: { tenant_id } });
        ...
        const total_amount = subtotal + total_cgst + total_sgst + total_igst;
        const cn = await prisma_1.default.creditNote.create({
            data: {
                tenant_id,
                cn_number,
                cn_date: new Date(),
...
const getSupplierBills = async (req, res) => {
    try {
        const tenant_id = req.user?.tenant_id;
        const bills = await prisma_1.default.supplierBill.findMany({
            where: { tenant_id },
            include: { lines: true, payments: true },
            orderBy: { created_at: 'desc' }
        });
...
exports.getSupplierBills = getSupplierBills;
const createSupplierBillFromGRN = async (req, res) => {
    try {
        const tenant_id = req.user?.tenant_id;
        const grn_id = req.params.grn_id;
        const grn = await prisma_1.default.grnHeader.findFirst({
        ...
        const po = grn.po_id ? await prisma_1.default.purchaseOrder.findFirst({ where: { id: grn.po_id } }) : null;
        const supplier = po?.supplier_id ? await prisma_1.default.supplierMaster.findUnique({ where: { id: po.supplier_id } }) : null;
        const config = await prisma_1.default.companyConfig.findUnique({ where: { tenant_id } });
        const factoryStateCode = config?.state_code || '33';
        const supplierStateCode = supplier?.state_code || '33';
        const isInterState = factoryStateCode !== supplierStateCode;
        let subtotal = 0;
        let total_cgst = 0;
        let total_sgst = 0;
        let total_igst = 0;
        const billLines = grn.grn_lines.map((gl) => {
        ...
        const total_amount = subtotal + total_cgst + total_sgst + total_igst;
        const count = await prisma_1.default.supplierBill.count({ where: { tenant_id } });
        const bill_number = `BILL-${new Date().getFullYear()}-${String(count + 1).padStart(4, '0')}`;
        const bill = await prisma_1.default.supplierBill.create({
            data: {
                tenant_id,
                bill_number,
                bill_date: new Date(),
...
exports.createSupplierBillFromGRN = createSupplierBillFromGRN;
const createPaymentVoucher = async (req, res) => {
    try {
        ...
        const bill = await prisma_1.default.supplierBill.findFirst({ where: { id: bill_id, tenant_id } });
        if (!bill)
            return res.status(404).json({ success: false, error: 'Bill not found' });
        const count = await prisma_1.default.paymentVoucher.count({ where: { tenant_id } });
        const voucher_number = `PV-${new Date().getFullYear()}-${String(count + 1).padStart(4, '0')}`;
        const voucher = await prisma_1.default.paymentVoucher.create({
            data: {
                tenant_id,
                voucher_number,
        ...
        await prisma_1.default.supplierBill.updateMany({
            where: { id: bill_id },
            data: { amount_paid: newAmountPaid, status: newStatus }
        });
        if (bank_account_id) {
...
const createDebitNote = async (req, res) => {
    try {
        ...
        const count = await prisma_1.default.debitNote.count({ where: { tenant_id } });
        const dn_number = `${prefix}-${new Date().getFullYear()}-${String(count + 1).padStart(4, '0')}`;
        const dn = await prisma_1.default.debitNote.create({
            data: {
                tenant_id,
                dn_number,
                dn_date: new Date(),
...
const getExpenses = async (req, res) => {
    try {
        ...
        const expenses = await prisma_1.default.expenseEntry.findMany({
            where: { tenant_id: tenant_id, is_reversed: false },
...
const createExpense = async (req, res) => {
    try {
        const tenant_id = req.user?.tenant_id;
        const count = await prisma_1.default.expenseEntry.count({ where: { tenant_id } });
        ...
        const expense = await prisma_1.default.expenseEntry.create({
...
const reverseExpense = async (req, res) => {
    try {
        ...
        const id = req.params.id;
        const { reason } = req.body;
        const expense = await prisma_1.default.expenseEntry.findFirst({ where: { id: id, tenant_id: tenant_id } });
        if (!expense)
            return res.status(404).json({ success: false, error: 'Expense not found' });
        await prisma_1.default.expenseEntry.updateMany({
            where: { id: id },
...
exports.reverseExpense = reverseExpense;
// ─── REPORTS ──────────────────────────────────────────────────────────────────
const getProfitAndLoss = async (req, res) => {
    try {
        ...
        const stockLedger = await prisma_1.default.stockLedger.findMany({
    ...
    try {
        ...
        for (const inv of invoices) {
            ...
            if (daysOutstanding > 30) {
                await prisma_1.default.invoiceHeader.updateMany({ where: { id: inv.id }, data: { status: 'overdue' } });
            }
        }
        const summary = {
            total_outstanding: Object.values(aging).reduce((s, a) => s + a.total_outstanding, 0),
            bucket_0_30: Object.values(aging).reduce((s, a) => s + a.buckets['0_30'], 0),
            bucket_31_60: Object.values(aging).reduce((s, a) => s + a.buckets['31_60'], 0),
            bucket_61_90: Object.values(aging).reduce((s, a) => s + a.buckets['61_90'], 0),
            bucket_90_plus: Object.values(aging).reduce((s, a) => s + a.buckets['90_plus'], 0)
        };
...
exports.getCustomerAging = getCustomerAging;
const getSupplierAging = async (req, res) => {
    try {
        ...
        const summary = {
            total_payable: Object.values(aging).reduce((s, a) => s + a.total_outstanding, 0),
            bucket_0_30: Object.values(aging).reduce((s, a) => s + a.buckets['0_30'], 0),
            bucket_31_60: Object.values(aging).reduce((s, a) => s + a.buckets['31_60'], 0),
            bucket_61_90: Object.values(aging).reduce((s, a) => s + a.buckets['61_90'], 0),
            bucket_90_plus: Object.values(aging).reduce((s, a) => s + a.buckets['90_plus'], 0)
...
exports.getSupplierAging = getSupplierAging;
const getGSTSummary = async (req, res) => {
    try {
        ...
        const output_cgst = invoices.reduce((s, i) => s + i.cgst_amount, 0);
        const output_sgst = invoices.reduce((s, i) => s + i.sgst_amount, 0);
        const output_igst = invoices.reduce((s, i) => s + i.igst_amount, 0);
        const cn_cgst = creditNotes.reduce((s, cn) => s + cn.cgst_amount, 0);
        const cn_sgst = creditNotes.reduce((s, cn) => s + cn.sgst_amount, 0);
        const cn_igst = creditNotes.reduce((s, cn) => s + cn.igst_amount, 0);
        const input_cgst = bills.reduce((s, b) => s + b.cgst_amount, 0);
        const input_sgst = bills.reduce((s, b) => s + b.sgst_amount, 0);
        const input_igst = bills.reduce((s, b) => s + b.igst_amount, 0);
...
const getCashPosition = async (req, res) => {
    try {
        const tenant_id = req.user?.tenant_id;
        const bankAccounts = await prisma_1.default.bankAccount.findMany({
        ...
        const receivablesDue = await prisma_1.default.invoiceHeader.findMany({
            where: { tenant_id, status: { in: ['sent', 'partial', 'overdue'] }, due_date: { lte: next30 } }
        });
        const totalReceivablesDue = receivablesDue.reduce((s, i) => s + (i.total_amount - i.amount_paid), 0);
        const payablesDue = await prisma_1.default.supplierBill.findMany({
            where: { tenant_id, status: { in: ['pending', 'partial'] }, due_date: { lte: next30 } }
        });
        const totalPayablesDue = payablesDue.reduce((s, b) => s + (b.total_amount - b.amount_paid), 0);
        const overdueReceivables = await prisma_1.default.invoiceHeader.findMany({
            where: { tenant_id, status: 'overdue' }
        });
        const totalOverdue = overdueReceivables.reduce((s, i) => s + (i.total_amount - i.amount_paid), 0);
        res.json({
            success: true,
            data: {
...
exports.getCashPosition = getCashPosition;
// ─── JOURNAL ENGINE ───────────────────────────────────────────────────────────
async function postJournalEntry(tenant_id, data) {
    const count = await prisma_1.default.journalEntry.count({ where: { tenant_id } });
    const entry_number = `JE-${new Date().getFullYear()}-${String(count + 1).padStart(5, '0')}`;
    const accounts = await prisma_1.default.chartOfAccount.findMany({ where: { tenant_id } });
    const getAccount = (code) => accounts.find((a) => a.account_code === code);
