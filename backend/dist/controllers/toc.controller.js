...
exports.getDashboard = exports.getShiftSummary = exports.getBufferStatus = exports.getPriorityQueue = exports.getConstraintConfig = exports.setConstraintConfig = exports.detectConstraint = void 0;
const prisma_1 = __importDefault(require("../config/prisma"));
const detectConstraint = async (req, res) => {
    try {
...
exports.detectConstraint = detectConstraint;
const setConstraintConfig = async (req, res) => {
    try {
        const tenant_id = req.user?.tenant_id;
        const { machine_id, buffer_target_hours, effective_from } = req.body;
        await prisma_1.default.constraintConfig.updateMany({
            where: { tenant_id, is_active_constraint: true },
            data: { is_active_constraint: false, effective_to: new Date() }
        });
        const config = await prisma_1.default.constraintConfig.create({
...
exports.setConstraintConfig = setConstraintConfig;
const getConstraintConfig = async (req, res) => {
    try {
        const tenant_id = req.user?.tenant_id;
        const config = await prisma_1.default.constraintConfig.findFirst({
            where: { tenant_id, is_active_constraint: true },
...
exports.getConstraintConfig = getConstraintConfig;
const getPriorityQueue = async (req, res) => {
    try {
        ...
        const queue = await Promise.all(activeJobs.map(async (job) => {
            const constraintOp = job.job_operations.find((op) => op.machine_id === constraint.id && !op.is_outsourced);
            if (!constraintOp)
                return null;
            ...
            if (job.item_id) {
                const item = await prisma_1.default.itemMaster.findUnique({ where: { id: job.item_id } });
...
exports.getPriorityQueue = getPriorityQueue;
const getBufferStatus = async (req, res) => {
    try {
        const tenant_id = req.user?.tenant_id;
        const config = await prisma_1.default.constraintConfig.findFirst({
            where: { tenant_id, is_active_constraint: true },
        ...
        for (const job of activeJobs) {
            ...
            const constraintOpIndex = ops.findIndex((op) => op.machine_id === config.machine_id);
            if (constraintOpIndex <= 0)
                continue;
            const opsBeforeConstraint = ops.slice(0, constraintOpIndex);
            const allCompleted = opsBeforeConstraint.every((op) => op.status === 'completed');
            if (allCompleted) {
                const machine = await prisma_1.default.machineMaster.findFirst({ where: { id: config.machine_id } });
                ...
                const pendingOp = opsBeforeConstraint.find((op) => op.status !== 'completed');
                if (pendingOp) {
                    blockedJobs.push({
                        job_number: job.job_number,
        ...
        await prisma_1.default.bufferLog.create({
            data: {
                tenant_id,
        ...
        const bufferHistory = await prisma_1.default.bufferLog.findMany({
...
const getShiftSummary = async (req, res) => {
    try {
        ...
        const totalPlanned = jobs.reduce((sum, j) => sum + j.planned_quantity, 0);
        const totalGood = jobs.reduce((sum, j) => sum + j.actual_quantity_good, 0);
        const totalShots = jobs.reduce((sum, j) => sum + j.shot_logs.length, 0);
        const totalRejections = jobs.reduce((sum, j) => sum + j.rejection_logs.length, 0);
        ...
        if (constraint) {
            const constraintJobs = jobs.filter((j) => j.machine_id === constraint.id);
            const cShots = constraintJobs.reduce((sum, j) => sum + j.shot_logs.length, 0);
            const cGood = constraintJobs.reduce((sum, j) => sum + j.actual_quantity_good, 0);
            const cDowntime = constraintJobs.reduce((sum, j) => sum + j.downtime_logs.reduce((s, d) => s + (d.duration_min || 0), 0), 0);
...
const getDashboard = async (req, res) => {
    try {
        ...
        const latestBuffer = config ? await prisma_1.default.bufferLog.findFirst({
            where: { tenant_id, machine_id: config.machine_id },
            orderBy: { logged_at: 'desc' }
        }) : null;
        let constraintOEE = null;
        if (config) {
            ...
            const constraintJobs = await prisma_1.default.jobCard.findMany({
                where: {
                    tenant_id,
                    machine_id: config.machine_id,
                    planned_date: { gte: startOfDay, lte: endOfDay }
            ...
            const totalShots = constraintJobs.reduce((sum, j) => sum + j.shot_logs.length, 0);
            const goodShots = constraintJobs.reduce((sum, j) => sum + j.actual_quantity_good, 0);
            const totalDowntime = constraintJobs.reduce((sum, j) => sum + j.downtime_logs.reduce((s, d) => s + (d.duration_min || 0), 0), 0);
            const shiftMin = 480;
