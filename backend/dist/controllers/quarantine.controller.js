...
const getQuarantineStock = async (req, res) => {
    try {
        ...
        const quarantine = await prisma_1.default.quarantineStock.findMany({
            where,
        ...
        const summary = {
            ...
            pending: quarantine.filter((q) => q.disposition === 'pending').length,
            total_qty: quarantine.reduce((s, q) => s + q.quantity, 0)
        };
...
exports.getQuarantineStock = getQuarantineStock;
const disposeQuarantineStock = async (req, res) => {
    try {
        const tenant_id = req.user?.tenant_id;
        const id = String(req.params.id);
        const { disposition, disposed_by, disposition_notes } = req.body;
        const quarantine = await prisma_1.default.quarantineStock.findFirst({ where: { id, tenant_id } });
        if (!quarantine)
            return res.status(404).json({ success: false, error: 'Quarantine record not found' });
        if (quarantine.disposition !== 'pending')
            return res.status(400).json({ success: false, error: 'Already disposed' });
        await prisma_1.default.quarantineStock.updateMany({
            where: { id, tenant_id },
